...
// AI: below section was generated by an AI
function handleColorPill(pill, col) {
	// Apply the color
	pC(col);
	...
	if (!container || !container.classList.contains('qcs-container')) return;

	// Remove active class from all pills
	d.querySelectorAll('.qcs.active').forEach(p => p.classList.remove('active'));

	// Add active class to this pill (shrinks it and shows the text via CSS)
	pill.classList.add('active');

	// Get the update text element for this container
	const updateText = container.querySelector('.qcs-update-text');
	if (updateText) {
		updateText.onclick = (e) => {
			e.stopPropagation();

			const selectedColor = cpick.color.rgb;
			let pid = currentPreset;

			if (pid > 0 && pJson[pid] && pJson[pid].seg && pJson[pid].seg[0]) {
				pJson[pid].seg[0].c1 = [selectedColor.r, selectedColor.g, selectedColor.b];

				const presetObj = _deepCopyPresets({[String(pid)]: pJson[pid]});
				const presetsToUpload = _deepCopyPresets(pJson);
				presetsToUpload[String(pid)] = presetObj[String(pid)];

				_uploadPresetsJson(presetsToUpload).then(() => {
					pJson = presetsToUpload;
					pill.classList.remove('active');
					showToast('Preset colours updated!', false, 2000, 'show-green');
				}).catch(err => {
					showToast('Failed to update preset: ' + err.message, true);
					pill.classList.remove('active');
				});
